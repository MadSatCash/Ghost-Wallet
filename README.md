...
- **The Destination Is Decoded, Not Trusted:** Both `cashaddr` (`bitcoincash:q...`) and legacy base58 (`1...` / `3...`) are accepted, and each is decoded down to the public key hash it will actually pay. The legacy form matters because it is what exchanges hand out for BCH deposits. Testnet addresses and Bitcoin `bc1...` bech32 are refused by name: a mainnet transaction to either one is valid, mined, and unspendable forever. And since a legacy address does not say which network it belongs to, the confirmation screen shows the `cashaddr` it resolved to before anything is signed.
